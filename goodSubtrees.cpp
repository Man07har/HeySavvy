...
class Solution{
public:
    vector<int>solve(Node* root, int& res, int k){
        if(!root){
            vector<int>temp(26, 0);
            return temp;
        }
        
        vector<int>left = solve(root -> left, res, k);
        vector<int>right = solve(root -> right, res, k);
        
        vector<int>arr(26, 0);
        arr[root -> data] = 1;
        
        for(int i=1;i<=25;i++){
            if(i == root -> data) continue;
    ...
    int goodSubtrees(Node *root,int k){
        int res = 0;
        vector<int>temp = solve(root, res, k);
